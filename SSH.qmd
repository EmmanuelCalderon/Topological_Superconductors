...
for i in range(Nt):
    cuadros.append( go.Frame(data=[go.Scatter(x=list(kdist), y=Ek[i][0],mode="lines"),
                                   go.Scatter(x=list(kdist), y=Ek[i][1],mode="lines")],
                             layout=go.Layout(title=f"$t_2/t_1={T2[i]:.2f}$" ) ))

cuadros=cuadros+list(reversed(cuadros))

fig = go.Figure(
    data=[go.Scatter(x=kdist, y=Ek[0][0],  mode="lines",
                     name=f'Valence band'),
          go.Scatter(x=kdist, y=Ek[0][1],mode="lines",
                     name=f'Conductance band')],
    layout=go.Layout(
        title=f"$t_2/t_1={T2[0]:.2f}$",
#        title=("$t_2/t_1=$"+str(T2[0])),
        xaxis_title="k",
        yaxis_title="E(k)",
...
In their paper (@Ryu_2006) S.Ryu and Y.Hatsugai proposed that the entanglement entropy could be used to characterize edge states and, as a consequence, topological phases. They also propose a novel way to calculate it, by means of a $C$ Hamiltonian. 
...
Now, from its definition, $P$ has only two eigenvalues, namely 0 and 1, and the same set of eigen wave functions as the original hamiltonian. This properties allow us to interpret the correlation matrix as a "hamiltonian" called the $C-$ Hamiltonian with periodic boundary conditions. We can observe how the $C-$ hamiltonian of the subsystem keeps both of these eigenvalues and as we aproach $|t_2/t_1|\rightarrow\infty$ all of the eigenvalues tend to 0 or 1. However, when we cut the system and obtain the matrix $C$, we allow for the formation of new states of this "hamiltonian" in the regime $|t_2/t_1|<1$. So the question is how to interpret the eigenstates.

 It is possible to solve analitically the system in the dimerized limit (for the SSH chain this corresponds to $t_2=0$), and in this regime Ryu and Hatsugai concluded the existence of two eigenstates with an eigenvalue equal to $1/2$ associated with the edge states. This is consistent with our previous calculations.

If $\{\zeta_\ell\}$ are the eigenvalues of $C$, we can calculate the etanglement entropy as
$$
